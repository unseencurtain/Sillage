...
| **Schedule** | Sync page: **Rebuild catalogue** + **Update prices & stock**; Settings **Minutes between syncs** (call interval). Rebuild queues when the schedule is on. Optional nightly rebuild under Schedule → Advanced. | Update hidden while Sync enabled is on; no silent disk “cache” sync |
