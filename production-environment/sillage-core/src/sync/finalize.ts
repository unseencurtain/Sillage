 ...
 * post counts are cached with no expiry, so a missed finalize leaves an import of 51,000 products
 * showing as an empty shop and an empty Products screen for as long as the shop stays up. That
 * happened on a first import where the bridge plugin had not been activated yet: the run reported
 * "51201 created" and the storefront showed nothing, with one WARN in between to explain it.
 *
