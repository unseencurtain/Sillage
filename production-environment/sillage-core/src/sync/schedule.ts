...
export function decide(
  ...
  // theme and shipping set up before fifty thousand products land in it, and an unattended seed on
  // a new VPS competes with WordPress for a small box's memory. Until one run has succeeded, every
  // tick declines and waits for Start on the dashboard.
  if (since === null) {
    return {
      action: "skip",
