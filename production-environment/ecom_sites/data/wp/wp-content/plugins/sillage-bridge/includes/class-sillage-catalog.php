...
final class Sillage_Catalog {
	...
	/**
	 * Top-level product_cat terms that have at least one published wholesale-perfumes product.
	 *
	 * The WPF feed's browse taxonomy is product *type* (Eau de Parfum, Blush, …). Brands belong
	 * on `product_brand`, not here — nested brand product_cat rows are ignored (parent != 0).
	 *
	 * @return array<int, array{term_id:int,name:string,slug:string,count:int}>
	 */
	private function get_b2b_category_terms(): array {
		$cached = get_transient( 'sillage_b2b_cats_v2' );
		if ( is_array( $cached ) ) {
			return $cached;
		}
		...
		set_transient( 'sillage_b2b_cats_v2', $terms, 15 * MINUTE_IN_SECONDS );
		return $terms;
	}
