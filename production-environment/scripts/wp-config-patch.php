<?php
/**
 * Patch wp-config.php from inside the `ecom` container.
 *
 * WordPress lives in a Docker volume, so the host cannot edit this file. Run it with:
 *
 *   docker cp scripts/wp-config-patch.php ecom:/tmp/
 *   docker exec -e SILLAGE_SHARED_SECRET=… ecom php /tmp/wp-config-patch.php
 *
...
if (!is_file($path)) {
    fwrite(STDERR, "WP_CONFIG_MISSING — start ecom first so WordPress can create wp-config.php\n");
    exit(1);
}
